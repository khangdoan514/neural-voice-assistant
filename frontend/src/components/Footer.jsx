    ...
    <footer className="bg-[#0D0B08] py-16 px-20 pt-10 pb-5 border-t border-rust/20">
      ...
      <div className="grid gap-16 mb-10" style={{ gridTemplateColumns: "3fr 1fr 1fr 1fr" }}>
        <div>
          ...
          <p className="text-body text-gray-500 leading-relaxed font-light mb-6">
        ...
        {FOOTER_SECTIONS.map(({ heading, links }) => (
          <div key={heading}>
            <h4 className="font-label text-label font-bold tracking-[3px] uppercase text-white mb-5">{heading}</h4>
            <ul className="flex flex-col gap-2.5 list-none p-0 m-0">
              {links.map(({ label, to }) => (
                <li key={label} className="m-0 p-0">
                  <Link
                    to={to}
                    className="transition-colors duration-200 text-gray-500 text-body hover:text-rust"
                  >
                    {label}
                  </Link>
                </li>
